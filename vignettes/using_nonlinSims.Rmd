...
num_measurements <- seq(from = 5, to = 11, by = 2)[3]
...
factor_list <- list(spacing = c("equal"),
                    num_measurements = seq(from = 9, to = 9, by = 2), 
...
test_convergence(factor_list = factor_list, num_iterations = 5, pop_params = pop_params, 
                 response_group_size = 225, num_cores = detectCores() - 1)
sfStop()
...
test_data #proof that same data set is being modeled 
model <- create_logistic_growth_model(data_wide = test_data, model_name = 'test')


##error occurs when determining good starting values; insert breakpoint at OpenMx:::match.barg(type)
mxAutoStart_local <- function (model, type = c("ULS", "DWLS")) 
{
  OpenMx:::warnModelCreatedByOldVersion(model)
  if (is(model$expectation, "MxExpectationGREML")) {
    return(autoStartGREML(model))
  }
  type <- OpenMx:::match.barg(type)
  defaultComputePlan <- (is.null(model@compute) || is(model@compute, 
                                                      "MxComputeDefault"))
...
#overwrite model with default starting values
model <- create_logistic_growth_model(data_wide = data_wide, model_name = 'test')

#error code is replicated
model <- mxAutoStart_local(model = model)
```
