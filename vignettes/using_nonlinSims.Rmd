...
packages <- c('tidyverse', 'data.table', 'nonlinSims', 'nlraa', "OpenMx", 'devtools')
libraries(packages)
...
beta_fixed <- (seq(from = 60, to = 300, by = 60))[4]
gamma_fixed <- 20
...
#starting_values <- generate_multiple_starting_value_sets(num_sets = 300, time_fixed_effects = c('beta_fixed', 'gamma_fixed'),
#                                            time_rand_effects = c('beta_rand', 'gamma_rand'), 
#                                            diff_effects = c('diff_fixed', 'diff_rand'), 
#                                            error = c('epsilon'))
#
#starting_values_184 <- starting_values[184, ]
#starting_values_184$beta_fixed <- 40.97266
#starting_values_184$gamma_fixed <- 44.43965
#starting_values_184$beta_rand <- 7.429643
#starting_values_184$gamma_rand <- 10.65619
#starting_values_184$diff_fixed <- 0.1386197
#starting_values_184$diff_rand <- 0.2855563
#starting_values_184$epsilon <- 0.8362621
...
param_table <- generate_ind_param_values(pop_param_list = pop_params, response_group_size = 30, num_time_points = num_measurements, cov_matrix = cov_matrix)

data <- generate_group_scores(num_measurements = num_measurements, param_table = param_table,measurement_days = schedule$measurement_days, time_period = 360, scaling_constant = 3)
...
starting_values <- generate_multiple_starting_value_sets(num_sets = 100, data = data, data_wide = data_wide)
starting_values$beta_rand <- starting_values$beta_fixed/2 + 5*runif(n = 100, min = -0.5, max = 0.5)
starting_values$gamma_rand <- starting_values$gamma_fixed/2 + 5*runif(n = 100, min = -0.5, max = 0.5)
...
fit_results <- evaluate_models(initial_model =  latent_growth_model, starting_values = starting_values)
fit_results

best_fitting_output <- extract_best_estimates(fit_results = fit_results, starting_values = starting_values)
best_fitting_output


mclapply(X = 10, FUN = compute_mult_simulations, pop_params = pop_params, cov_matrix = cov_matrix, schedule = schedule, response_group_size = 30, mc.cores = 3)

compute_mult_simulations(num_sims = 50, pop_params = pop_params, cov_matrix = cov_matrix, schedule = schedule, response_group_size = 30)
```

```{r echo=F}
tic()
mclapply(X = 1:10, FUN = compute_mult_simulations, pop_params = pop_params, cov_matrix = cov_matrix, schedule = schedule, response_group_size = 30, mc.cores = detectCores() -1)
toc()
```
...
```{r testing-starting-value procedure, echo=F}
#failsafe guard on starting value procedure 
##generate another set of starting values if if no set of starting value
#with cell size n = 100, test convergence rate for all three conditions of experiment 1 (spacing, shape, number of measurements)
##sample size is kept constant at 200?
