...
packages <- c('devtools', 'nonlinSims', 'parallel', 'tidyverse', "OpenMx", "data.table")
libraries(packages)
...
pop_params <- generate_three_param_pop_curve(diff_fixed = 0.32, beta_fixed =  beta_fixed = beta_fixed, gamma_fixed = gamma_fixed,
                                             sd_diff = 0)
...
run_exp_simulation_4l(factor_list = factor_list, num_iterations = 2, pop_params = pop_params_4l, response_group_size = 225, num_cores = 3, seed = 27)




cov_matrix_4l <- generate_four_param_cov_matrix(num_time_points = num_time_points, pop_param_list = pop_params_4l)

schedule <- compute_measurement_schedule(time_period = 360, num_measurements = num_time_points, smallest_int_length = 30, measurement_spacing = 'equal')

ind_param_values_4l <- generate_4l_ind_param_values(pop_param_list = pop_params_4l, response_group_size = response_group_size, 
                                                num_time_points = num_time_points,  cov_matrix = cov_matrix_4l)

data <-generate_group_scores_4l(num_measurements = num_time_points, param_table = ind_param_values_4l, measurement_days = schedule$measurement_days, time_period = time_period)
...
#get good starting values for theta, alpha, beta, and gamma 
fixed_starts <- getInitial(obs_score ~ SSfpl(input = measurement_day, A = theta, B = alpha, xmid = beta, scal = gamma), data = data)
start_value_output <- nlsList(obs_score ~ SSfpl(input = measurement_day, A = theta, B = alpha, xmid = beta, scal = gamma)| ID, data = data, 
                                start = fixed_starts, warn.nls = F)
ind_estimated_values <- ranef(start_value_output)
  
#theta_rand starting values  
ordered_theta <- rand_starts$theta[order(rand_starts$theta)]
middle_95_percentiles_theta <- quantile(ordered_theta, probs = c(.025, .975), na.rm = T)
middle_95_theta <- na.exclude(ordered_theta[ordered_theta >= middle_95_percentiles_theta[1] & ordered_theta <= middle_95_percentiles_theta[2]])

#alpha_rand starting values  
ordered_alpha <- rand_starts$alpha[order(rand_starts$alpha)]
middle_95_percentiles_alpha <- quantile(ordered_alpha, probs = c(.025, .975), na.rm = T)
middle_95_alpha <- na.exclude(ordered_alpha[ordered_alpha >= middle_95_percentiles_alpha[1] & ordered_alpha <= middle_95_percentiles_alpha[2]])

#beta_rand starting values  
ordered_beta <- rand_starts$beta[order(rand_starts$beta)]
middle_95_percentiles_beta <- quantile(ordered_beta, probs = c(.025, .975), na.rm = T)
middle_95_gamma<- na.exclude(ordered_beta[ordered_diff >= middle_95_percentiles_beta[1] & ordered_beta <= middle_95_percentiles_beta[2]])

#gamma_rand starting values  
ordered_gamma <- rand_starts$gamma[order(rand_starts$gamma)]
middle_95_percentiles_gamma <- quantile(ordered_gamma, probs = c(.025, .975), na.rm = T)
middle_95_gamma<- na.exclude(ordered_gamma[ordered_diff >= middle_95_percentiles_gamma[1] & ordered_gamma <= middle_95_percentiles_gamma[2]])
...
start_values <- list('theta_fixed' = fixed_starts['theta'], 
                     ...
                     'theta_rand' = var(middle_95_theta), #var(middle_95_diff), 
                     'alpha_rand' = var(middle_95_alpha), #var(middle_95_diff), 
                     'beta_rand' = var(middle_95_beta), 
                     'gamma_rand' = var(middle_95_gamma), 
...
tic()
ouput_2<-run_exp_simulation(factor_list = factor_list, num_iterations = 1, pop_params = pop_params, response_group_size = 225, num_cores = 3, seed = 27)
toc()
